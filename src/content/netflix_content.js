...
(function () {
  ...
  function openSidebarList() {
    ...
    closeBtn.onclick = () => { isLoopSidebarOn = false; closeSidebar(); };
  ...
  async function fetchListAndRender(container) {
    try {
      ...
      if (!items.length) { container.textContent = 'データがありません。'; return; }
      container.innerHTML = '';
      for (const item of items) {
        ...
        jump.style.cssText = 'margin-top:4px;background:#0f0;color:#000;border:none;padding:4px 8px;cursor:pointer;';
  ...
  function openSidebarRecord(data) {
    ...
    closeBtn.textContent = '×';
    closeBtn.style.cssText = 'background:red;color:#fff;border:none;cursor:pointer;font-size:14px;';
    closeBtn.onclick = () => { closeSidebar(); videoEl?.play?.(); };
    header.append(title, closeBtn);
    ...
    save.textContent = '保存';
    save.style.cssText = 'background:#00c853;border:none;color:#fff;padding:6px;cursor:pointer;';
    save.onclick = () => {
  ...
  function sendRecord(payload) {
    ...
    }).then(r=>r.json()).then(j=>console.log('保存OK', j)).catch(e=>console.error('保存NG', e));
  }
  ...
  function handleRecordClick() {
    try {
      ...
      if (isRecording) {
        ...
        if (recordStart > recordEnd) throw new Error('録画終了時刻が開始時刻より早い値です');
        if (Math.abs(recordEnd - recordStart) < 1) throw new Error('録画範囲が短すぎます');
        ...
        // リセット
        isRecording = false; recordStart = null; recordEnd = null;
      } else {
        isRecording = true;
  ...
  async function selectClip(clipId) {
    try {
      ...
      // Cookie保存
      ["title","user","startTime","endTime","url","service"].forEach(k=>{
  ...
  function redirectToClip({ url, service, startTime }) {
    if (!url || !service) { alert('URL または サービス情報が不正です'); return; }
    let base;
    switch (String(service).toLowerCase()) {
      case 'netflix': base = `https://www.netflix.com${url}`; break;
      case 'amazon':
      case 'prime video': base = `https://www.amazon.co.jp${url}`; break;
      case 'youtube': base = `https://www.youtube.com${url}`; break;
      default: alert(`未対応のサービス: ${service}`); return;
    }
    const finalUrl = base + (base.includes('?') ? '&' : '?') + 't=' + Math.floor(startTime || 0);
  ...
  async function playNextClip() {
    ...
    const currentClipId = (document.cookie.match(/(?:^|; )clipId=([^;]*)/)||[])[1] || '000000';
    const userId = (document.cookie.match(/(?:^|; )username=([^;]*)/)||[])[1] || 'anonymous';
    try {
    ...
    return new Promise((resolve, reject) => {
      chrome.storage.local.get(['playClipSystemKey','clip'], (res) => {
        ...
        if (res.playClipSystemKey === 1 && res.clip) {
          clipData = res.clip; console.info('[Clip] loaded:', clipData);
        } else { clipData = null; }
        resolve();
      });
    });
  ...
  function createButtonsIfNeeded() {
    ...
    if (controls && episodeBtn && fwd10 && (!hasRecord || !hasSidebar || !hasNext)) {
      // ラッパ
      ...
      if (recordSvg) { recordBtn.appendChild(recordSvg); }
      ...
      nextBtn.addEventListener('click', () => {
        ...
        if (nextSvg?.style) nextSvg.style.color = nextClipToggleOn ? COLORS.active : COLORS.default;
      ...
      toggleBtn.addEventListener('click', () => {
        ...
        if (detailSvg?.style) detailSvg.style.color = isLoopSidebarOn ? COLORS.active : COLORS.default;
  ...
  async function initMain() {
    try {
      ...
      videoEl = await (async ()=>{
        const v = q('video');
        return v || await new Promise(resolve=>{
          const mo = new MutationObserver(()=>{
            const nv = q('video');
            if (nv) { mo.disconnect(); resolve(nv); }
          });
